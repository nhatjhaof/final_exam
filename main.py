...
try:
    ...
    #Load đến thư mục env ở file này
    load_dotenv()
    ...
    t0 = time.time()
    ssh_tunnel.start()
    print(f"SSH tunnel start took {time.time() - t0:.3f}s")
    ...
    print(f"MySQL connect took {time.time() - t1:.3f}s")

# Sau khi tạo db_connection và cursor
    try:
        cursor.execute("SELECT 1;")
        result = cursor.fetchone()
        if result and list(result.values())[0] == 1:
            print("✅ Kết nối DB thành công và trả về:", result)
        else:
            print("❌ Kết nối DB thất bại hoặc kết quả không đúng:", result)
    except mysql.connector.Error as err:
            print("❌ Lỗi khi test kết nối:", err)
            raise
